...
logger = logging.getLogger("keeper_engine.service.assess")
    ...
    def assess(self, req: AssessRequest, on_progress: Callable[[], None] | None = None) -> AssessResponse:
        ...
        def work(idx: int) -> None:
            ...
            try:
                ...
                # 完整 traceback 落日志（含出错的依赖与行号），便于定位「哪行炸的」；
                # 用户面只回简短 `类型: 消息`，不把堆栈塞进 errors。
                logger.exception("层①评分失败 path=%s", photo.path)
